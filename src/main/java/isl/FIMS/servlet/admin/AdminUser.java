...
public class AdminUser extends AdminBasicServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (this.action.equals("list")) {
            ...
            StringBuffer querySource = new StringBuffer("for $i in doc('" + this.adminDbCollection + this.conf.USERS_FILE + "')//user");
            querySource.append("\nlet $groupname := string(doc('" + this.adminDbCollection + this.conf.GROUPS_FILE + "')//group[@id=$i//groups/group]/@groupname)");
            ...
            StringBuffer querySource = new StringBuffer("for $i in doc('" + this.adminDbCollection + this.conf.USERS_FILE + "')//user[@id='" + id + "']");
            querySource.append("\nlet $groupname := string(doc('" + this.adminDbCollection + this.conf.GROUPS_FILE + "')//group[@id=$i//groups/group]/@groupname)");
            ...
            if (this.displayMsg.length() == 0) {
                ...
                if (isSysAdmin == true) {
                    String usernameTmp = DMSUser.getUsernameOf(Integer.parseInt(id), this.conf);
                    String rights = this.getRights(usernameTmp);
            ...
            if (!orgId.equals("0") && !orgId.equals(userOrg) && !mode.equals("sys")) {
                ...
                String usernameInsert = request.getParameter("username");
                ...
                if (this.displayMsg.length() == 0) {
                    try {
                        ...
                        if (this.action.equals("insert")) {
                            ...
                            if (df.exist("/DMS/users/user[./@id!='" + id + "']/info/email[./text()='" + email + "']")) {
                                displayMsg += Messages.Email_EXIST;
                                throw new EntryExistException("User already exists: " + email);
                            } else {
